...
I considered three different ways to create charts in Android device
  ...
  1. [Google chart API][1]
  2. [Achartengine][2]
  3. [ChartDroid][3] I tried creating a Pie Chart while using the solutions. Google charting API being the easiest one to integrate is taken first
...
[caption id="attachment_4674" align="alignnone" width="300" caption="Google Charting API PIE Chart"]![Google Charting API PIE Chart][5][/caption]

The only issue I had with this approach is that the device should have internet access available which in my case may not be always there. This small limitation made me look into other approaches and I came across another solution [Achartengine][2].

#### Achartengine

This Achart's charting support comes in the form of a download-able [Jar][6].

I added this into the lib folder of my application.

[caption id="attachment_4677" align="alignnone" width="247" caption="Android App Structure"]![Library Folder][7][/caption]
...
Next step is to create a CategorySeries with the appropriate data. Once this is done we can get achartingengine specific intent back with the following call ` ChartFactory.getPieChartIntent(context, categorySeries, renderer); `
...
@Override public String getType(Uri uri) { return "vnd.android.cursor.dir/vnd.com.googlecode.chartdroid.graphable"; }
...
MatrixCursor c = new MatrixCursor(new String[] { BaseColumns._ID, "COLUMN_AXIS_INDEX", "COLUMN_SERIES_INDEX", "COLUMN_DATUM_VALUE", "COLUMN_DATUM_LABEL" });

c.newRow().add(1).add(0).add(1).add(30).add(null); c.newRow().add(2).add(0).add(1).add(10).add(null);

   [1]: http://code.google.com/apis/chart/
   [2]: http://www.achartengine.org/
   [3]: http://code.google.com/p/chartdroid/
   [4]: http://code.google.com/apis/chart/docs/making_charts.html
   [5]: http://xebee.xebia.in/wp-content/uploads/2010/08/GoogleAPIChart-300x193.png (GoogleAPIChart)
   [6]: http://code.google.com/p/achartengine/downloads/list
   [7]: http://xebee.xebia.in/wp-content/uploads/2010/08/libFolder.png (libFolder)
   [8]: http://xebee.xebia.in/wp-content/uploads/2010/08/Achart2-223x300.png (Achart)
   [9]: http://code.google.com/p/chartdroid/downloads/list
